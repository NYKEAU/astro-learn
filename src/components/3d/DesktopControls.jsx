...
export function DesktopControls({
    ...
    // Générer l'URL accessible depuis mobile
        ...
        <>
            <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
                ...
                <button
                    ...
                    aria-label={language === 'fr' ? 'Envoyer sur mobile' : 'Send to mobile'}
                >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
                    </svg>
                    {language === 'fr' ? 'Sur mobile' : 'To mobile'}
                    {isLoading && <div className="w-3 h-3 border border-white/30 border-t-white rounded-full animate-spin"></div>}
                </button>
                    ...
                    <div className="flex flex-col gap-1">
                        {animationModes.map((mode) => (
                            <button
                                ...
                                className={`px-3 py-1 rounded text-xs transition-colors ${mode.active
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-lg font-bold text-lunar-white">
                                    {language === 'fr' ? 'Partager sur mobile' : 'Share to mobile'}
                                </h3>
                                <button
                                    onClick={() => setShowQRModal(false)}
                            ...
                            <div className="flex mb-4 bg-cosmic-black/50 rounded-lg p-1">
                                <button
                                    ...
                                    className={`flex-1 py-2 px-3 rounded text-xs transition-colors ${shareMode === 'qr'
                                            ? 'bg-neon-blue text-cosmic-black'
                                ...
                                <button
                                    ...
                                    className={`flex-1 py-2 px-3 rounded text-xs transition-colors ${shareMode === 'code'
                                            ? 'bg-neon-blue text-cosmic-black'
                                            : 'text-lunar-white/70 hover:text-lunar-white'
                                        }`}
                                >
                                ...
                                <>
                                    ...
                                    <div className="mt-3 p-3 bg-cosmic-black/50 rounded-lg">
                                        <div className="text-xs text-lunar-white/70 mb-1">
                                            {language === 'fr' ? 'Adresse réseau :' : 'Network address:'}
                                        </div>
                                        <div className="text-xs text-lunar-white/90 font-mono break-all">
                                            {localIP === 'localhost' ? (
                                                <span className="text-yellow-400">
                                                    {language === 'fr' ? 'Localhost uniquement' : 'Localhost only'}
                                                </span>
                                            ) : (
                                ...
                                <>
                                    ...
                                    <div className="text-center mb-4">
                                        ...
                                        <p className="text-sm text-lunar-white/80 mb-2">
                                            {language === 'fr'
                                                ? "Entrez ce code sur votre mobile"
                                                : "Enter this code on your mobile"
                                            }
                                        </p>
                                        <p className="text-xs text-lunar-white/60">
                                            {language === 'fr'
                                                ? "Rendez-vous sur astrolearn.com/share"
                                                : "Go to astrolearn.com/share"
                                            }
                                        </p>
                                    </div>
                                    ...
                                    <div className="mt-3 p-3 bg-cosmic-black/50 rounded-lg">
                                        <p className="text-xs text-lunar-white/60">
                                            ⏱️ {language === 'fr' ? 'Expire dans 10 minutes' : 'Expires in 10 minutes'}
                                        </p>
                                    </div>
                                </>
                            )}
